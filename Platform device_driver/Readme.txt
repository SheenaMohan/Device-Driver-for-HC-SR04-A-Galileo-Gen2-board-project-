...
1) To configure the MUX gpio pins:
	The gpio mux pins specified in the F and G column of "gpio_io pins" excel sheet(attached in the folder) are virtual pins whose direction is by default "out".
...
eg :if(0< write(PinExport,"66",2)) line has to be changed to
	if(0< write(PinExport,"56",2)) if the linux gpio MUX is 56
	
	if(0< write(PinUnexport,"66",2)) line has to be changed to
	if(0< write(PinUnexport,"56",2)) if the linux gpio MUX is 56
...
3) After configuring the gpio linux mux pins, follow the steps to set the platform device and driver for the HCSR sensor.
	...
	After the command "make all" .platform_device.ko,platform_driver.ko, hcsr_tester and hcsr_sys files are gernerated.Load thesealong with hcsr_sysfs.sh
	onto the board.These files are present in the executables folder.
	...
	1)insmod platform_device.ko
	2) insmod platform driver.ko	(the steps 1 and 2 can be interchanged)

	After the platform device and the driver are matched, the driver is probed to intialise and register the device. Since there are two sensors,
 	the driver probe function is called twice. simultaneously the class HCSR, the device subdirectories HCSRdevice1 and HCSRdevice2 and their respective attributes are created.
	
	3)now we can run our application using the dev interface by :
			chmod 777 hcsr_tester
			./hcsr_tester
	4) we can also run the sysfs interface by :
			chmod 777 hcsr_sysfs
			./hcsr_sysfs
	5) Since we have configured the mux pins at the user space to test the dev interface in step 3, we can run step 4 without setting the mux pins again, but if step 4 has to be 
	   tested first then we can run:
			chmod 777 hcsr_sys
